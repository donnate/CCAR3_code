...
script_path <-"~/Documents/CCAR3_code"# get_script_path()
  ...
  for (nnzeros in nnzero_values){
    ...
    #for (p in c(20, 50, 80, 100, 200, 500, 1000)){
      ...
      for (r in rs){
              ...
              for(prop_missing in props){
                ...
                tryCatch({
                  start_time_alt3 <- system.time({
                    res_alt = ccar3_methods$cca_rrr_cv(gen$X, gen$Y,
                                                       r = r,
                                                       lambdas = c(10^seq(-3, 0, length.out = 15)),
                                                       kfolds = 5,
                                                       solver = "ADMM",
                                                       parallelize = TRUE,
                                                       LW_Sy = LW_Sy,
                ...
                tryCatch({
                  start_time_alt4 <- system.time({
                                        res_alt1 <- ccar3_methods$cca_rrr(gen$X, gen$Y, Sx = NULL, Sy = NULL,
                                                                          lambda = 0.2,
                                                                          r = r,
                                                                          solver = "ADMM",
                                                                          LW_Sy = LW_Sy,       
		...
		for (method in c("FIT_SAR_CV" 
		                 #"FIT_SAR_BIC", "Witten_Perm","Witten.CV", "Waaijenborg-Author", "Waaijenborg-CV", "SCCA_Parkhomenko", "Fantope", "Chao", "SGCA"
		                 )){
                    ...
                    start_time_additional_method <- system.time({
                      if (method %in% ccar3_benchmark_methods) {
                        ...
                        test1 <- additional_checks(gen$X,
                                                   ...
                                                   rank=r, kfolds=5, 
                                                   method.type = method,
                                                   lambdax= 10^seq(-5,1, length.out = 30),
                                                   lambday = 10^seq(-5,1, length.out = 30))
                      }
